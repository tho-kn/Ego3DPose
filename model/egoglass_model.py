...
class EgoGlassModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = [
            ...
            'segmentation_left', 'segmentation_right',
    ...
    def set_input(self, data):
        ...
        self.gt_segmentation_left = data['gt_segmentation_left'].cuda(self.device)
        self.gt_segmentation_right = data['gt_segmentation_right'].cuda(self.device)

    def forward(self):
        with autocast(enabled=self.opt.use_amp):
            self.pred_heatmap_left, self.pred_segmentation_left = self.net_HeatMap_left(self.input_rgb_left)
            self.pred_heatmap_right, self.pred_segmentation_right = self.net_HeatMap_right(self.input_rgb_right)
    ...
    def backward_HeatMap(self):
        with autocast(enabled=self.opt.use_amp):
            ...
            loss_segmentation_left = self.lossfunc_MSE(
                self.pred_segmentation_left, self.gt_segmentation_left
            )
            loss_segmentation_right = self.lossfunc_MSE(
                self.pred_segmentation_right, self.gt_segmentation_right
            )
            ...
            self.loss_segmentation_left = loss_segmentation_left * self.opt.lambda_segmentation
            self.loss_segmentation_right = loss_segmentation_right * self.opt.lambda_segmentation
            
            loss_total = self.loss_heatmap_left + self.loss_heatmap_right + self.loss_segmentation_left + self.loss_segmentation_right
    ...
    def evaluate(self, runnning_average_dict):
        ...
        return self.pred_pose, pred_heatmap_cat, runnning_average_dict